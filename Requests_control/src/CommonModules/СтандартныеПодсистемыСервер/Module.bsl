...
Функция ДобавитьПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	ПерсональныеНастройкиРаботыСФайлами = Новый Структура;
	РаботаСФайламиВызовСервера.УстановитьПерсональныеНастройкиРаботыСФайлами(ПерсональныеНастройкиРаботыСФайлами);
	Параметры.Вставить("ПерсональныеНастройкиРаботыСФайлами", ПерсональныеНастройкиРаботыСФайлами);
	
	КоличествоЗанятыхФайлов = 0;
	Параметры.Вставить("КоличествоЗанятыхФайлов", 0);
	
	Параметры.Вставить("РезервноеКопированиеИБПриЗавершенииРаботы", РезервноеКопированиеИБСервер.ПараметрыПриЗавершенииРаботы());
	
	Параметры.Вставить("ЗапрашиватьПодтверждениеПриЗавершенииПрограммы", СтандартныеПодсистемыСервер.ЗапрашиватьПодтверждениеПриЗавершенииПрограммы());
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РегламентныеЗадания") Тогда
		МодульРегламентныеЗаданияСлужебный = ОбщегоНазначения.ОбщийМодуль("РегламентныеЗаданияСлужебный");
		Параметры.Вставить("ПоказатьФормуБлокировкиРаботыСВнешнимиРесурсами",
			МодульРегламентныеЗаданияСлужебный.УстановленаБлокировкаРаботыСВнешнимиРесурсами(ЭтоВызовПередНачаломРаботыСистемы));
	КонецЕсли;
...
Процедура ПередЗапускомПрограммы()
	...
	МинимальнаяВерсияПлатформы = "8.3.7.1759";
	...
	Если ЗначениеЗаполнено(НедоступныйРежим) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Режим совместимости конфигурации с 1С:Предприятием версии %1 не поддерживается.
			           ...
			           |версии не ниже 8.3.7 или ""Не использовать"".'"),
			НедоступныйРежим);
	КонецЕсли;
