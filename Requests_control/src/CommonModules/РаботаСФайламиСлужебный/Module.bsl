...
Процедура ПриПолученииФайла(ЭлементДанных, ПолучениеЭлемента) Экспорт
	...
	Попытка
		...
		ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.ВерсииФайлов") Тогда
			...
			Если НЕ ЭлементДанных.ЭтоНовый() Тогда
				...
				Если ВерсияФайла.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
					...
					ВременныйПуть = ПолучитьИмяВременногоФайла(ВерсияФайла.Расширение);
					
					Попытка
						ПереместитьФайл(ПрежнийПутьНаТоме, ВременныйПуть);
					Исключение
						// не обрабатываем исключение, даже если с файлами была ошибка
						// возможно, в предыдущем цикле приема была ошибка.
						ЗаписьЖурналаРегистрации(
							НСтр("ru = 'ПриПолученииФайла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
							УровеньЖурналаРегистрации.Ошибка,,,
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					КонецПопытки;
					
					ДанныеВосстановления = Новый Структура("СтарыйПуть, ВременныйПуть", 
						ПрежнийПутьНаТоме, ВременныйПуть);
		...
		РаботаСФайламиВызовСервера.ЗафиксироватьТранзакциюДляФайловВТоме(ДанныеДляВосстановленияФайловВТоме);
