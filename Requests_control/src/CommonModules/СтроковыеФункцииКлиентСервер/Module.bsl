...
// Проверяет существование в строке отдельно стоящего слова.
//
Функция СтрокаСодержитСлово(Строка, Слово) Экспорт
	
	Разделители = " -_/\.,;`""" + Символы.ПС + Символы.Таб + Символы.ВК;
	
	НачальнаяПозиция = 1;
	Пока НачальнаяПозиция <= СтрДлина(Строка) Цикл
		Вхождение = СтрНайти(ВРег(Строка), ВРег(Слово), НаправлениеПоиска.СНачала, НачальнаяПозиция);
		Если Вхождение = 0 Тогда
			Прервать;
		КонецЕсли;
		СлеваОтдельно = (Вхождение = 1
			Или ЭтоРазделительСлов(Сред(Строка, Вхождение - 1)));
		ПозицияКонца = Вхождение + СтрДлина(Слово) - 1;
		СправаОтдельно = (ПозицияКонца = СтрДлина(Строка)
			Или ЭтоРазделительСлов(КодСимвола(Сред(Строка, ПозицияКонца + 1))));
		Если СлеваОтдельно И СправаОтдельно Тогда
			Возврат Истина;
		КонецЕсли;
		НачальнаяПозиция = ПозицияКонца + 1;
	КонецЦикла;
