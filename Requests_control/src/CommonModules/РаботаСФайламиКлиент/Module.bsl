...
Функция СоздатьДокументНаОсновеФайла(
	...
	СведенияОФайле.Ответный = Ответный;
...
Процедура ИнформироватьОШифровании(МассивФайловВРабочемКаталогеДляУдаления, ВладелецФайла,
	...
	Оповестить(
		...
		Новый Структура("Событие, Файл, Владелец, ЕстьЗанятыеФайлы, ИдентификаторРодительскойФормы", 
			"ДанныеФайлаИзменены", 
			ФайлСсылка, ВладелецФайла, ЕстьЗанятыеФайлы,
			Неопределено),
			ФайлСсылка);
...
//  ФайлСсылка  - СправочникСсылка.Файлы - файл.
//  ЕстьЗанятыеФайлы  - Булево - 
Процедура ИнформироватьОРасшифровке(ВладелецФайла, ФайлСсылка, ЕстьЗанятыеФайлы) Экспорт
	...
	Если ПараметрыВыполнения.ДанныеФайла.Размер <> РазмерФайла Тогда
		...
		Если Не ПараметрыВыполнения.ДанныеФайла.Свойство("ПутьОбновленияИзФайлаНаДиске") Тогда
			...
			Оповестить("Запись_Файл",
				...
				"ДанныеФайлаИзменены", ПараметрыВыполнения.ИдентификаторФормы),
				ПараметрыВыполнения.ДанныеФайла.Ссылка);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОчиститьРабочийКаталог(ОбработчикРезультата, РазмерФайловВРабочемКаталоге, РазмерДобавляемогоФайла, 
		ОчищатьВсе, НеВыдаватьСообщений = Ложь) Экспорт
...
Процедура ОсвободитьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	//Обработчик = Новый ОписаниеОповещения("ОсвободитьФайлыПоСсылкамПослеОтветаНаВопросОтменитьРедактирование", 
	//	ЭтотОбъект, ПараметрыВыполнения);
	//
	//ПоказатьВопрос(
	//	Обработчик,
...
Процедура ЗакончитьРедактированиеПоСсылкам(
	...
	НеПоказыватьОшибки = Ложь) Экспорт
	...
	ПараметрыОбработчика.Вставить("НеПоказыватьОшибки", НеПоказыватьОшибки);
...
Процедура ЗакончитьРедактированиеПоСсылкамИтерация(Результат, ПараметрыВыполнения) Экспорт
	...
	ПараметрыОбработчика.НеПоказыватьОшибки = ПараметрыВыполнения.НеПоказыватьОшибки;
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПроверкиЗашифрован(ПараметрыВыполнения)
	...
	Исключение
		...
		Если ПараметрыВыполнения.НеПоказыватьОшибки Тогда
			
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				РаботаСФайламиКлиентСервер.СобытиеЖурналаРегистрации(),
...
Функция ПараметрыОбновленияФайла(ОбработчикРезультата, ОбъектСсылка, ИдентификаторФормы) Экспорт
	...
	ПараметрыОбработчика.Вставить("НеПоказыватьОшибки", Ложь);
...
Процедура СохранитьИзмененияФайлаБезРасширенияПослеЗагрузкиФайла(ПомещенныеФайлы, ПараметрыВыполнения) Экспорт
	...
	Результат = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаИСохранитьИзмененияФайла(ПараметрыВыполнения.ОбъектСсылка, СведенияОФайле, 
...
Процедура ДобавитьФайл(
	...
	ПараметрыВыполнения.Вставить("Ответный", Ответный);
...
Процедура ДобавитьПослеВыбораРежимаСоздания(РежимСоздания, ПараметрыВыполнения) Экспорт
	...
	Если РежимСоздания = 2 Тогда // Загрузить из файловой системы.
...
Функция ПолучитьФильтрДляДиалогаВыбораФайла() Экспорт
	...
	Фильтр 
		= НСтр("ru = 'Все файлы (*.*)|*.*"
		+ "|Документы MS Office(*.doc;*.docx;*.xls;*.xlsx;*.ppt;*.pptx)|*.doc;*.docx;*.xls;*.xlsx;*.ppt;*.pptx"
		+ "|Документы Open Office(*.odt;*.odf;*.ods;*.odp)|*.odt;*.odf;*.ods;*.odp"
		+ "|Текстовые файлы(*.txt)|*.txt"
		+ "|Файлы PDF(*.pdf)|*.pdf"
		+ "|Файлы HTML(*.htm;*.html)|*.htm;*.html"
		+ "|Картинки (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf"
		+ "'");
...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
		...
		Иначе
			...
			Для Каждого ИмяФайла из МассивИменФайлов Цикл
				...
				СоздатьДокументНаОсновеФайла(
					...
					ПараметрыВыполнения.ФормаВладелец, 
					НеОткрыватьКарточкуПослеСозданияИзФайла,,,,
	...
	Попытка
		...
		СведенияОФайле.ПараметрыРаспознавания = ПараметрыРаспознавания;
		СведенияОФайле.Ответный = ПараметрыВыполнения.Ответный;
...
Процедура ВыбратьРежимИРедактироватьФайл(ОбработчикРезультата, ДанныеФайла, ДоступностьКомандыРедактировать) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеФайла.Редактирует)
		...
		// упрощенный режим - сразу редактируем
		РаботаСФайламиКлиент.ВернутьРезультат(ОбработчикРезультата, РезультатРедактировать);
		Возврат;
		
	ИначеЕсли ЗначениеЗаполнено(ДанныеФайла.Редактирует)
