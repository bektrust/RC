...
Функция ПолучитьИнтернетПочта(ПараметрыЗагрузки) Экспорт
	...
	Соединение = Почта.ИнтернетПочтаУстановитьСоединение(УчетнаяЗапись, Пароль, СообщениеОбОшибке, ИспользоватьДляПолучения);
	...
	ИнтернетПочтаПараметрыЗагрузки.ПараметрыОтбора.Вставить("ПослеДатыОтправления", ТекущаяДата() - 3 * 86400);
...
Функция ПолучитьНастройкиПрофилейДляОтправки() Экспорт
	...
	УчетныеЗаписи = Почта.ПолучитьУчетныеЗаписиЭлектроннойПочты();
