...
//
Процедура ПриСозданииНаСервере(Форма, ДополнительныеПараметры = Неопределено, НаборСвойств = Неопределено) Экспорт
	
	Если НаборСвойств = Неопределено 
		И Не ИспользуютсяСвойства(Форма, ДополнительныеПараметры) Тогда
		Возврат;
	КонецЕсли;
	...
	Контекст.Вставить("ПоложениеЗаголовка",         Неопределено);
	...
	Если Контекст.Свойство("ПоложениеЗаголовка") Тогда
		КонтекстПоложениеЗаголовка = Контекст.ПоложениеЗаголовка;
	Иначе
		КонтекстПоложениеЗаголовка = Неопределено;
	КонецЕсли;
	
	Если Не Контекст.ПроизвольныйОбъект
		И Не Контекст.ОтложеннаяИнициализация Тогда
		ЗаполнитьДополнительныеРеквизитыВФорме(Форма, ОписаниеОбъекта, , Контекст.СкрытьУдаленные, КонтекстПоложениеЗаголовка);
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено,
	ПоложениеЗаголовка = Неопределено) Экспорт
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов)
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			Если Пользователи.РолиДоступны("Пользователь,ПолныеПрава") Тогда
				// Добавление команды.
