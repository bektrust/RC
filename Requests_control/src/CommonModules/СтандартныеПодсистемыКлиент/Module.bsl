...
#КонецЕсли
	...
	Для Каждого Обработчик Из ОбработчикиСобытия Цикл
		Обработчик.Модуль.ПриПолученииСпискаПредупрежденийЗавершенияРаботы(Отказ, Предупреждения);
	КонецЦикла;
...
// Только для внутреннего использования из процедуры ПоказатьРезультатВыполнения.
...
Функция ВыполнитьШаг(Сценарий)
	...
	ИначеЕсли Сценарий.ТекущийШаг = "ВывестиПредупреждение" Тогда
		Возврат ВывестиПредупреждение(Сценарий);
	...
	ИначеЕсли Сценарий.ТекущийШаг = "РазвернутьУзлыДерева" Тогда
		Возврат РазвернутьУзлыДерева(Сценарий);
...
// Только для внутреннего использования.
Функция ВывестиПредупреждение(Сценарий)
	ВыводПредупреждения = Сценарий.Параметры;
	Если Сценарий.МестоОстановки = Неопределено Тогда
		
		КнопкаПоУмолчанию = КодВозвратаДиалога.ОК;
		
		Кнопки = Новый СписокЗначений;
		Если ЗначениеЗаполнено(ВыводПредупреждения.Подробно) Тогда
			КнопкаПоУмолчанию = 1;
			Кнопки.Добавить(1, НСтр("ru = 'Подробнее...'"));
		КонецЕсли;
		Если ТипЗнч(Сценарий.Форма) = Тип("УправляемаяФорма") И ЗначениеЗаполнено(ВыводПредупреждения.ПутьКРеквизитуФормы) Тогда
			Кнопки.Добавить(2, НСтр("ru = 'Перейти к реквизиту'"));
		КонецЕсли;
		Кнопки.Добавить(КодВозвратаДиалога.ОК, НСтр("ru = 'Закрыть'"));
		
		ПоказатьВопрос(Обработчик(Сценарий), ВыводПредупреждения.Текст, Кнопки, , КнопкаПоУмолчанию, ВыводПредупреждения.Заголовок);
		Сценарий.МестоОстановки = "ЗаданВопрос";
		Возврат Ложь; // Приостановить сценарий.
		
	ИначеЕсли Сценарий.МестоОстановки = "ЗаданВопрос" Тогда
		
		Ответ = Сценарий.Результат;
		Если ТипЗнч(Ответ) = Тип("Число") Тогда
			Если Ответ = 1 Тогда
				
				ПолныйТекст = Строка(ВыводПредупреждения.Текст) + Символы.ПС + Символы.ПС + ВыводПредупреждения.Подробно;
				ПоказатьПодробнуюИнформацию(Обработчик(Сценарий), ПолныйТекст, ВыводПредупреждения.Заголовок);
			...
			ИначеЕсли Ответ = 2 Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.ИдентификаторНазначения = Сценарий.Форма.УникальныйИдентификатор;
				Сообщение.Текст = ВыводПредупреждения.Текст;
				Сообщение.Поле  = ВыводПредупреждения.ПутьКРеквизитуФормы;
				Сообщение.Сообщить();
...
// Только для внутреннего использования.
Функция РазвернутьУзлыДерева(Сценарий)
	РазворачиваемыйУзел = Сценарий.Параметры;
	ТаблицаЭлемент = Сценарий.Форма.Элементы[РазворачиваемыйУзел.ИмяТаблицы];
	Если РазворачиваемыйУзел.Идентификатор = "*" Тогда
		Узлы = Сценарий.Форма[РазворачиваемыйУзел.ИмяТаблицы].ПолучитьЭлементы();
