...
Функция УстановленаБлокировкаРаботыСВнешнимиРесурсами(ПоказатьФорму = Ложь) Экспорт
	
	Если ПоказатьФорму И Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		Возврат Ложь; // Форма показывается только администратору.
	КонецЕсли;
	...
	Иначе // Клиент-серверная база
		ИмяБазы = СтроковыеФункцииКлиентСервер.ПолучитьПараметрыИзСтроки(СтрокаСоединения).Ref;
		СохраненноеИмяБазы = СтроковыеФункцииКлиентСервер.ПолучитьПараметрыИзСтроки(ПараметрыБлокировки.СтрокаСоединения).Ref;
		Если ИмяБазы <> СохраненноеИмяБазы Или ИмяКомпьютера() <> ПараметрыБлокировки.ИмяКомпьютера Тогда
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки);
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
...
//
Процедура ИнициализироватьПараметрыБлокировкиРаботыСВнешнимиРесурсами() Экспорт
	...
	ПараметрыБлокировки.Вставить("РаботаСВнешнимиРесурсамиЗаблокирована", Ложь);
...
Процедура УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки)
