...
Процедура ДобавитьНастройкиРаботыСФайлами(ОбщиеНастройки, ПерсональныеНастройки) Экспорт
	...
	ПерсональныеНастройки.Вставить("ЕстьИспользованныеШаблоны", Ложь);
	
	ПредупреждатьОЗаполненииФайлов = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ДоговорныеДокументы", "ПредупреждатьОЗаполненииФайлов");
	Если ПредупреждатьОЗаполненииФайлов = Неопределено Тогда
		ПредупреждатьОЗаполненииФайлов = Истина;
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ДоговорныеДокументы", "ПредупреждатьОЗаполненииФайлов", ПредупреждатьОЗаполненииФайлов);
	КонецЕсли;
	ПерсональныеНастройки.Вставить("ПредупреждатьОЗаполненииФайлов", ПредупреждатьОЗаполненииФайлов);
