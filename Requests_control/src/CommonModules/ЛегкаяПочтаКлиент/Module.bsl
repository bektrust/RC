...
Функция ПолучитьMSOutlook(ПараметрыЗагрузки) Экспорт
	...
	Для каждого Item Из Items Цикл
		...
		Исключение	
			ПоказатьПредупреждение(, НСтр("ru = 'Требуется разрешить доступ к Microsoft Outlook'"));
			Возврат Результат;
		КонецПопытки;	
...
Функция ПометитьКакПрочтенныеMSOutlook(
	...
	Попытка 
		ВерсияСтрока = Application.Version;
		МассивСтрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ВерсияСтрока, ".");
		Если МассивСтрок.Количество() > 0 Тогда
			ПервоеЧислоВерсии = МассивСтрок[0];
			ВерсияOutlook = Число(ПервоеЧислоВерсии);
...
// Настроить почту и выполнить команду - отправку или приемку
Процедура НастроитьПочтуИВыполнить(ФинальныйОбработчик, ПараметрКоманды, ДляОтправки = Истина) Экспорт
	
	ПараметрыВыполнения = Новый Структура("ПараметрКоманды, ФинальныйОбработчик", 
		ПараметрКоманды, ФинальныйОбработчик);
	...
	ПараметрыФормы.Вставить("ЭтоНастройкаДляЗагрузки", Не ДляОтправки);
...
Процедура ПослеВопросаОНастройкеПочты(КодВозврата, ПараметрыВыполнения) Экспорт
	
	Если КодВозврата = КодВозвратаДиалога.Да Тогда
		
		Обработчик = Новый ОписаниеОповещения("ПослеВопросаОВыбореВариантаОтправки", 
			ЭтотОбъект, ПараметрыВыполнения);
		
		ОткрытьФорму("Обработка.НастройкаПочты.Форма.ВыборВариантаОтправки", 
...
Процедура ПослеВопросаОВыбореВариантаОтправки(КодВозврата, ПараметрыВыполнения) Экспорт
	
	Если КодВозврата = ПредопределенноеЗначение("Перечисление.ВидыПочтовыхКлиентов.ИнтернетПочта") Тогда
		
		Обработчик = Новый ОписаниеОповещения("ПослеВопросаОВыбореИнтернетПочты", ЭтотОбъект, ПараметрыВыполнения);
	...
	ИначеЕсли КодВозврата = ПредопределенноеЗначение("Перечисление.ВидыПочтовыхКлиентов.MSOutlook") Тогда
		
		ВидПочтовогоКлиента = ПредопределенноеЗначение("Перечисление.ВидыПочтовыхКлиентов.MSOutlook");
		
		ПрофилиДляОтправкиХранилище = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("РаботаСПочтой", "ПрофилиДляОтправки");	
		Если ТипЗнч(ПрофилиДляОтправкиХранилище) <> Тип("СписокЗначений") Тогда
			ПрофилиДляОтправкиХранилище = Новый СписокЗначений;
		КонецЕсли;
		ПрофилиДляЗагрузкиХранилище = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("РаботаСПочтой", "ПрофилиДляЗагрузки");	
		...
		Данные = Новый Структура("Путь", "");
		СтруктураПрофиля.Вставить("Данные", Данные);
		ЛегкаяПочтаКлиентСервер.СохранитьПрофиль(ПрофилиДляОтправкиХранилище, СтруктураПрофиля);
		ЛегкаяПочтаКлиентСервер.СохранитьПрофиль(ПрофилиДляЗагрузкиХранилище, СтруктураПрофиля);
		
		МассивСтруктур = Новый Массив;
		ДобавитьСтруктуруНастройки(МассивСтруктур, "РаботаСПочтой", "ПрофильДляОтправки", ПрофильСтр);
		ДобавитьСтруктуруНастройки(МассивСтруктур, "РаботаСПочтой", "ПрофилиДляОтправки", ПрофилиДляОтправкиХранилище);
		ДобавитьСтруктуруНастройки(МассивСтруктур, "РаботаСПочтой", "ПрофильДляЗагрузки", ПрофильСтр);
		...
		ВыполнитьОбработкуОповещения(ПараметрыВыполнения.ФинальныйОбработчик, ПараметрыВыполнения.ПараметрКоманды);
...
Процедура ПослеВопросаОВыбореИнтернетПочты(КодВозврата, ПараметрыВыполнения) Экспорт
	...
	Если СохранитьНастройкиПочты(КодВозврата) Тогда
		ВыполнитьОбработкуОповещения(ПараметрыВыполнения.ФинальныйОбработчик, ПараметрыВыполнения.ПараметрКоманды);
	КонецЕсли;	
	
КонецПроцедуры	

Функция СохранитьНастройкиПочты(КодВозврата) Экспорт
	
	Если ТипЗнч(КодВозврата) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") Тогда
		
		ВидПочтовогоКлиента = ПредопределенноеЗначение("Перечисление.ВидыПочтовыхКлиентов.ИнтернетПочта");
		
		УчетнаяЗапись = КодВозврата;
		
		МассивСтруктур = Новый Массив;
		ДобавитьСтруктуруНастройки(МассивСтруктур, "РаботаСПочтой", "ПрофильДляОтправки", УчетнаяЗапись);
		ДобавитьСтруктуруНастройки(МассивСтруктур, "РаботаСПочтой", "ПрофильДляЗагрузки", УчетнаяЗапись);
	...
	ИначеЕсли ТипЗнч(КодВозврата) = Тип("Строка") Тогда	
		...
		СтруктураПрофиля.Вставить("ВидПочтовогоКлиента", ВидПочтовогоКлиента);
		СтруктураПрофиля.Вставить("Использовать", Истина);
		Данные = Новый Структура("Путь", КодВозврата);
		СтруктураПрофиля.Вставить("Данные", Данные);
		ЛегкаяПочтаКлиентСервер.СохранитьПрофиль(ПрофилиДляОтправкиХранилище, СтруктураПрофиля);
		ЛегкаяПочтаКлиентСервер.СохранитьПрофиль(ПрофилиДляЗагрузкиХранилище, СтруктураПрофиля);
