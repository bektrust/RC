...
Процедура УстановкаПараметровСеанса(Знач ИмяПараметра, УстановленныеПараметры) Экспорт
		...
		Иначе
			...
			Иначе
				...
				Если РезультатПользователи.Пустой() Тогда
					...
					Если Пользователи.ЭтоПолноправныйПользователь( , Не РазделениеВключено, Ложь) Тогда
						ИдентификаторПользователяИБ = ТекущийПользовательИБ.УникальныйИдентификатор;
	...
	Если ТекущийПользователь = Неопределено
	 ИЛИ ТекущийВнешнийПользователь = Неопределено Тогда
		...
		ВызватьИсключение ТекстСообщенияПользовательНеНайденВСправочнике(ИмяПользователя) + Символы.ПС
...
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь) Экспорт
	...
	Если ПользовательПоИдентификаторуСуществует(
		...
		Если ПриНачалеРаботыСистемы
		   И Пользователи.ЭтоПолноправныйПользователь(ТекущийПользовательИБ, Не РазделениеВключено, Ложь) Тогда
			
			ИнтеграцияСтандартныхПодсистем.ПриАвторизацииАдминистратораПриНачалеРаботыСистемы(НайденныйПользователь);
		КонецЕсли;
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры, Отказ) Экспорт
	...
	ОшибкаАвторизации = АвторизоватьТекущегоПользователя(Истина);
	...
	Если ТребуетсяСменитьПароль(ОшибкаАвторизации, Истина) Тогда
...
//
Функция ПарольОтвечаетТребованиямСложности(Пароль) Экспорт
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РеквизитыПользователя.Недействителен <> Ложь Тогда
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	ДополнительныеПараметры.Вставить("ПарольУстановлен", ПользовательИБ.ПарольУстановлен);
...
Процедура ЗавершитьОбработкуПользователяИБ(ПользовательОбъект, ПараметрыОбработки) Экспорт
	...
	Если ПараметрыОбработки.Свойство("ЗаписьАдминистратора") Тогда
		УстановитьПривилегированныйРежим(Истина);
		ИнтеграцияСтандартныхПодсистем.ПриЗаписиАдминистратора(ПользовательОбъект.Ссылка);
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
Функция ТребуетсяСменитьПароль(ОписаниеОшибки = "", ПриЗапуске = Ложь)
		...
		Если ПриЗапуске Тогда
			...
			ЗаписьЖурналаРегистрации(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось записать сведения о пользователе ""%1"" по причине:
					           |%2'"),
					ТекущийПользователь, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ЗаписатьПользователяИБ(ПользовательОбъект, ПараметрыОбработки)
	...
	Если Не ПараметрыОбработки.СтарыйПользовательИБСуществует
	   И ОписаниеПользователяИБ.Свойство("Пароль")
	   И ОписаниеПользователяИБ.Свойство("ЗаполнитьПароль")
	   И ОписаниеПользователяИБ.Пароль = Неопределено
	   И ОписаниеПользователяИБ.ЗаполнитьПароль Тогда
		
		СложныйПароль = ПолучитьПроверкуСложностиПаролейПользователей();
		ДлинаПароля   = ПолучитьМинимальнуюДлинуПаролейПользователей();
		...
		Если Настройки.ОбщиеНастройкиВхода Тогда
			...
			КонецЕсли;
			СложныйПароль = СложныйПароль Или НастройкиВхода.ПарольДолженОтвечатьТребованиямСложности;
			Если ДлинаПароля < НастройкиВхода.МинимальнаяДлинаПароля Тогда
				ДлинаПароля = НастройкиВхода.МинимальнаяДлинаПароля;
			КонецЕсли;
		КонецЕсли;
		
		Если СложныйПароль Или ДлинаПароля > 0 Тогда
			ДлинаПароля = ?(ДлинаПароля < 7, 7, ДлинаПароля);
			
			ОписаниеПользователяИБ.Пароль = СоздатьПароль(ПараметрыПароля(ДлинаПароля, СложныйПароль));
		КонецЕсли;
	КонецЕсли;
