...
Процедура ОсвободитьФайл(ДанныеФайла, УникальныйИдентификатор = Неопределено) Экспорт
	...
	ДанныеФайла.Вставить("ЕстьЗанятыеФайлы", ЕстьЗанятыеФайлы(ДанныеФайла.Владелец));
...
Функция СоздатьФайл(Знач Владелец, Знач СведенияОФайле) Экспорт
	...
	Файл.Ответный = СведенияОФайле.Ответный;
...
// АдресВременногоХранилищаТекста - Строка - содержит адрес во временном хранилище, где находятся двоичные данные с файлом текста или ХранилищеЗначения - непосредственно содержит  двоичные данные с файлом текста
//  УникальныйИдентификатор - уникальный идентификатор формы
Процедура ОбновитьТекстВФайле(ФайлСсылка, Знач АдресВременногоХранилищаТекста, УникальныйИдентификатор = Неопределено) Экспорт
...
Функция СохранитьИзмененияИОсвободитьФайл(ДанныеФайла, СведенияОФайле,
	...
	Попытка
		...
		Если НоваяВерсия <> Неопределено И НЕ ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент()
			И Не НеМенятьЗаписьВРабочемКаталоге Тогда
...
Функция ДанныеФайла(ФайлСсылка, ВерсияСсылка = Неопределено) Экспорт
...
Функция ПолучитьДанныеФайлаИЗанятьФайл(ФайлСсылка, ДанныеФайла, СтрокаОшибки, ДатаЗаема, 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Заем файла для редактирования'", КодОсновногоЯзыка), 
...
Функция ПолучитьДанныеФайлаДляОткрытияИЗанятьФайл(ФайлСсылка, ДанныеФайла, СтрокаОшибки, 
	...
	Попытка
		...
		ДанныеФайла.Вставить("ЕстьЗанятыеФайлы", 
			ЕстьЗанятыеФайлы(ДанныеФайла.Владелец));
	...
	Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
		Если НЕ ВерсияСсылка.Том.Пустая() Тогда
			...
			Исключение
				...
				КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Открытие файла'", КодОсновногоЯзыка), 
		...
		Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске И ДанныеФайла.Версия <> Неопределено Тогда
			...
			Если НЕ ДанныеФайлаТом.Пустая() Тогда
				...
				Исключение
					...
					КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение данных файла для открытия'", КодОсновногоЯзыка), 
						УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.Файлы, ДанныеФайла.Ссылка, СообщениеОбОшибке);
...
Функция УстановитьВладельцаФайла(МассивСсылокНаФайлы, НовыйВладелецФайла) Экспорт
	...
	Если ТипЗнч(НовыйВладелецФайла) <> ТипЗнч(ПрежнийВладелец) Тогда 
		...
		СкопироватьФайлы(МассивСсылокНаФайлы, НовыйВладелецФайла);
...
Функция РаспознатьФайл(ФайлСсылка, ПараметрыРаспознавания, ОписаниеОшибки, РаспознанныйТекст, 
	...
	РеквизитыФайла = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ФайлСсылка, 
		"Зашифрован, ТекущаяВерсияРасширение, ТекущаяВерсия");
...
Функция ПолучитьВсеПодчиненныеФайлы(ВладелецФайла, ПолучатьПомеченныеНаУдаление = Истина,
	НеПоказыватьОтветные = Ложь, ТолькоОтветные = Ложь) Экспорт
	...
	Если НеПоказыватьОтветные И Не ТолькоОтветные Тогда
		Запрос.Текст = Запрос.Текст + " И Файлы.Ответный = ЛОЖЬ";
	КонецЕсли;
	
	Если ТолькоОтветные Тогда
		Запрос.Текст = Запрос.Текст + " И Файлы.Ответный = ИСТИНА";
	КонецЕсли;
...
Процедура УстановитьПерсональныеНастройкиРаботыСФайлами(Настройки) Экспорт
	...
	//Настройки.Вставить("ЭтоПолноправныйПользовательИБ", ПользователиСерверПовтИсп.ЭтоПолноправныйПользовательИБ());
...
// Вернет Истина, если есть занятые файлы у этого Владельца
Функция ЕстьЗанятыеФайлы(Документ) Экспорт
...
Процедура ВыполнитьУдалениеНеактивнойВерсииФайла(ВерсияСсылка, Параметры, ИмяКаталогаСохранения,
	...
	Попытка
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
		ЗаписьЖурналаРегистрации(НСтр("ru='Выполнена очистка неактивной версии файла'", КодОсновногоЯзыка), 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
		ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка очистки неактивной версии файла'", КодОсновногоЯзыка), 
			УровеньЖурналаРегистрации.Ошибка, 
...
// Обновляет файл
// ФайлСсылка - существующий файл, СведенияОФайле - данные нового файла
Процедура ОбновитьФайл(ФайлСсылка, СведенияОФайле, УникальныйИдентификатор) Экспорт
	
	ДанныеФайлаТекущие = РаботаСФайламиВызовСервера.ДанныеФайла(ФайлСсылка);
	...
	Попытка
		...
		НоваяВерсия = РаботаСФайламиВызовСервера.ОбновитьВерсиюФайла(
			ФайлСсылка, СведенияОФайле, ,	УникальныйИдентификатор);
		...
		Если НоваяВерсия <> Неопределено Тогда
			...
			Если СведенияОФайле.ХранитьВерсии Тогда
				РаботаСФайламиВызовСервера.ОбновитьВерсиюВФайле(ФайлСсылка, НоваяВерсия, СведенияОФайле.АдресВременногоХранилищаТекста, 
					УникальныйИдентификатор);
			Иначе
				РаботаСФайламиВызовСервера.ОбновитьТекстВФайле(ФайлСсылка, СведенияОФайле.АдресВременногоХранилищаТекста, 
					УникальныйИдентификатор);
			КонецЕсли;
...
// Получает файлы, занятые текущим пользователем
Функция ПолучитьСписокЗанятыхФайлов() Экспорт
