...
// объектов зарегистированных для отложенного обновления в главном узле РИБ
//
Процедура ОбработатьДанныеДляОбновленияВПодчиненномУзле() Экспорт
	...
	Если Не СтандартныеПодсистемыПовтИсп.ИспользуетсяРИБ("СФильтром")
		И Не ЭтоПодчиненныйУзелРИБ()
		И НазначениеПланаОбмена(ГлавныйУзел().Метаданные().Имя) <> "РИБСФильтром" Тогда
		Возврат;
	КонецЕсли;
...
// Выполняет закрытие XML файла с записаной информацией об объектах
...
Функция ПолучитьДополнительныеСвойстваПланаОбменаСтрокой(Знач СвойстваСтрокой)
	...
	СвойстваМассив = СтрРазделить(СвойстваСтрокой, ",");
