...
Процедура РассылкаОтчетовПросмотрПапки(СтандартнаяОбработка, Папка) Экспорт
	...
	ОткрытьФорму("Справочник.Файлы.Форма.ФормаСписка", ПараметрыФормы, , Папка);
...
Процедура ИмпортФайловЦиклПродолжитьИмпорт(ПараметрыВыполнения)
	...
	Если Не ПараметрыВыполнения.ПапкаУжеНайдена Тогда
		ПараметрыВыполнения.ПапкаДляДобавленияТекущая = РаботаСФайламиВызовСервера.СправочникиПапкиСоздатьЭлемент(
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ПрикладнаяФормаПредупреждения = "Справочник.Файлы.Форма.СписокЗанятыхСВопросомПриЗавершенииРаботы";
...
//
Процедура ПриПолученииСпискаПредупрежденийЗавершенияРаботы(Отказ, Предупреждения) Экспорт
	...
	ПредупреждениеПользователю.ТекстГиперссылки = СтрШаблон(
