...
Процедура ВариантОтчетаПриЗаписиПриЗаписи(Источник, Отказ) Экспорт
	...
	//Этот вариант отчета имеет ключ заданный в конфигураторе и соответствует ключу из родителя верхнего уровня. 
...
//   ПараметрыЗаполнения - настройки, которые необходимо применить к отчету
//
Процедура УстановитьНастройкиОтчетаПоУмолчанию(МенеджерОтчета, ПараметрыЗаполнения) Экспорт
	
	Если ТипЗнч(ПараметрыЗаполнения) =  Тип("Структура") И 
			ПараметрыЗаполнения.Свойство("НастройкаВариантаОтчета") Тогда
		
		НастройкаВариантаОтчета = ПараметрыЗаполнения.НастройкаВариантаОтчета;
		...
		Исключение
			//В настройках по умолчанию отсутствуют снимок отчета и макет отчета
...
Функция ПолучитьРазделыОтчета(Отчет)
	...
	Раздел = Метаданные.Подсистемы.Настройки;
...
Процедура УстановитьПризнакВспомогательногоОтчета(НастройкаВариантаОтчета)
	...
	Если ИмяОтчета = "АнализВерсийОбъектов" 
		...
		Или ИмяОтчета = "ГруппыДоступаСНеограниченнымиПравами"
		...
		Или ИмяОтчета = "УниверсальныйОтчет"
	Тогда
...
// Процедура заполняет по умолчанию признак ИспользоватьДляКонтекстныхОтчетов для категорий отчетов
//
// Параметры
//   нет
//
Процедура ЗаполнитьСвойстваКатегорий() Экспорт
	
	Категории = Справочники.КатегорииОтчетов.Выбрать();
	
	Пока Категории.Следующий() Цикл
		
		Подстрока = Найти(Категории.ИмяПредопределенныхДанных, "Список");
		
		Если Подстрока > 0 Тогда
			
			Объект = Категории.ПолучитьОбъект();
			Объект.ИспользоватьДляКонтекстныхОтчетов = Истина;
			Объект.Записать();
