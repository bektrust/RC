...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
	...
	Если Результат.СсылкаВарианта = Неопределено Тогда
		Если Результат.СсылкаОтчета = Неопределено
			Или Результат.КлючВарианта = Неопределено Тогда
	...
	Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов")
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В метод ""%1"" передан некорректный параметр ""%2"" (тип ""%3"", значение ""%4"").'"),
			"ПодключитьОтчетИЗагрузитьНастройки",
			"СсылкаВарианта",
			Строка(ТипЗнч(Результат.СсылкаВарианта)),
	...
	Если Результат.СсылкаОтчета = Неопределено
		Или Результат.КлючВарианта = Неопределено Тогда
		СвойстваВарианта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Результат.СсылкаВарианта, "Отчет, КлючВарианта");
	...
	Если Результат.Объект = Неопределено Тогда
		...
		Если Не ПодключениеОтчета.Успех Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Вариант ""%1"" (ключ ""%2""): %3'"),
				Строка(Результат.СсылкаВарианта),
				Результат.КлючВарианта,
				ПодключениеОтчета.ТекстОшибки);
			Возврат Результат;
		КонецЕсли;
	...
	// Чтение настроек.
	Результат.НастройкиКД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "НастройкиКД");
	Если ЗначениеЗаполнено(Результат.КлючВарианта) И ТипЗнч(Результат.НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Результат.НастройкиКД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Результат.СсылкаВарианта, "Настройки").Получить();
	...
	Результат.АдресСхемы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "АдресСхемы");
	Результат.ПользовательскиеНастройкиКД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ПользовательскиеНастройкиКД");
	
	АдресСхемыЗаполнен = (ТипЗнч(Результат.АдресСхемы) = Тип("Строка") И ЭтоАдресВременногоХранилища(Результат.АдресСхемы));
	СхемаКД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "СхемаКД");
	Если АдресСхемыЗаполнен И ТипЗнч(СхемаКД) <> Тип("СхемаКомпоновкиДанных") Тогда
		СхемаКД = ПолучитьИзВременногоХранилища(Результат.АдресСхемы);
	КонецЕсли;
	
	Результат.СхемаМодифицирована = (ТипЗнч(СхемаКД) = Тип("СхемаКомпоновкиДанных"));
...
Процедура ПриОпределенииНазначенияРолей(НазначениеРолей) Экспорт
	...
	НазначениеРолей.СовместноДляПользователейИВнешнихПользователей.Добавить(
		Метаданные.Роли.Пользователь.Имя);
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	ВыполнятьЗамеры = ИмяКлючевойОперации <> "" И ВыполнятьЗамеры();
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш)
	...
	|			ИЛИ ЕСТЬNULL(ВариантыОтчетов.Отчет.ПометкаУдаления, ИСТИНА) = ИСТИНА)
	|	И ВариантыОтчетов.ТипОтчета = &ТипОтчета";
