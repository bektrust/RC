...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВладелецФайла") Тогда
		...
		Если ТипЗнч(Параметры.ВладелецФайла) = Тип("Строка") Тогда
			ИмяВызывающейФормы = Параметры.ВладелецФайла;
		КонецЕсли;	
	...
	Если Параметры.Свойство("Ответный") Тогда
		Ответный = Параметры.Ответный;
	КонецЕсли;
...
Процедура ЗагрузитьНастройки()
	...
	Если ПоказыватьДиалогСканераЗагрузка = Неопределено Тогда
		ПоказыватьДиалогСканераЗагрузка = Ложь;
...
Процедура СканироватьЕще(Команда)
	...
	ПоказыватьДиалог = ПоказыватьДиалогСканера;
	ВыбранноеУстройство = ИмяУстройстваСканирования;
	ПутьКПрограммеКонвертации = "magick.exe";
	ПараметрСжатие = ?(ВРег(ФорматКартинки) = "JPG", КачествоJPG, СжатиеTIFFЧисло);
...
Процедура ПринятьВсеКакОтдельныеФайлы(Команда)
	...
	Для Каждого Строка Из МассивФайловКопия Цикл
		...
		Если НЕ ПустаяСтрока(ПутьКФайлуЛокальный) Тогда
			...
			Иначе
				...
				МассивФайлов.Добавить(Новый Структура("ИмяФайла, ПутьФайла", ИмяФайла, ПутьКФайлуЛокальный));
	...
	Если Не ЗначениеЗаполнено(ВладелецФайла) И МассивФайлов.Количество() <> 0 Тогда
		
		Оповестить(
			"Сканирование_Файла", 
			Новый Структура("МассивФайлов, Владелец", 
				МассивФайлов, 
				ИмяВызывающейФормы));
...
#КонецЕсли	
	...
	Если НЕ ПустаяСтрока(ПараметрыВыполнения.ФайлРезультата) Тогда
		...
		Если ЗначениеЗаполнено(ВладелецФайла) Тогда			
			...
			ПараметрыДобавления.Вставить("Ответный", Ответный);
		...
		Иначе
			...
			Оповестить(
				"Сканирование_Файла", 
				Новый Структура("ПутьФайла, Владелец, ИмяФайла", 
					ПараметрыВыполнения.ФайлРезультата, 
					ИмяВызывающейФормы,
					ИмяФайла));
