...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТребуетсяЗаписьПользователяИБ(ЭтотОбъект)
	 Или ПараметрыЗаписи.Свойство("КраткоеИмя") Тогда
		
		ОписаниеПользователяИБ = ОписаниеПользователяИБ();
		Если ПараметрыЗаписи.Свойство("КраткоеИмя") Тогда
			ОписаниеПользователяИБ.Вставить("Имя", ПараметрыЗаписи.КраткоеИмя);
		КонецЕсли;
		...
		ОписаниеПользователяИБ.Вставить("ЗаполнитьПароль", Истина);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("СменитьПарольПослеЗаписи") Тогда
		ПодключитьОбработчикОжидания("СменитьПарольНачало", 0.1, Истина);
	КонецЕсли;
...
Процедура СменитьПароль(Команда)
	...
	Если Не Модифицированность
	   ...
	   И ПользовательИБСуществует Тогда
	...
	Если Не ЗначениеЗаполнено(Объект.Наименование)
	 Или ЗначениеЗаполнено(Объект.Ссылка)
	   И ПользовательИБСуществует Тогда
		...
		СменитьПарольПослеПодтвержденияЗаписи("Записать", Контекст);
		Возврат;
	КонецЕсли;
	
	// Текст вопроса в стиле 1С:Предприятия.
	Если ЗначениеЗаполнено(ПользовательИБИмя) Тогда
		ТекстВопроса = НСтр("ru = 'Установка пароля возможна только после записи карточки пользователя.'");
	Иначе
		Контекст.Вставить("КраткоеИмя",
			ПользователиСлужебныйКлиентСервер.ПолучитьКраткоеИмяПользователяИБ(Объект.Наименование));
		
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Установка пароля возможна только после записи поля
				           |Имя (для входа), которое будет установлено: ""%1"".'"),
				Контекст.КраткоеИмя);
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Установка пароля возможна только после записи карточки пользователя,
				           |при этом будет установлено Имя (для входа): ""%1"".'"),
				Контекст.КраткоеИмя);
		КонецЕсли;
	КонецЕсли;
	...
	Кнопки.Добавить("НеЗаписывать", НСтр("ru = 'Не записывать'"));
	
	ПоказатьВопрос(Новый ОписаниеОповещения("СменитьПарольПослеПодтвержденияЗаписи", ЭтотОбъект, Контекст),
		ТекстВопроса, Кнопки);
...
&НаКлиенте
Процедура СменитьПарольПослеПодтвержденияЗаписи(Ответ, Контекст) Экспорт
	
	Если Ответ <> "Записать" Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыЗаписи = Новый Структура("СменитьПарольПослеЗаписи");
	
	Если Контекст.Свойство("КраткоеИмя") Тогда
		ПараметрыЗаписи.Вставить("КраткоеИмя", Контекст.КраткоеИмя);
	КонецЕсли;
...
// Продолжение процедуры СменитьПароль.
&НаКлиенте
Процедура СменитьПарольНачало()
	
	ПользователиСлужебныйКлиент.ОткрытьФормуСменыПароля(Объект.Ссылка, Новый ОписаниеОповещения(
		"СменитьПарольПослеСменыПароля", ЭтотОбъект));
	
КонецПроцедуры

// Продолжение процедуры СменитьПароль.
&НаКлиенте
Процедура СменитьПарольПослеСменыПароля(Результат, Контекст) Экспорт
