...
&НаКлиенте
Процедура СписокДополнительныхРеквизитовПриАктивизацииСтроки(Элемент)
	Если ТипЗнч(Элемент.ТекущаяСтрока) = Тип("ПланВидовХарактеристикСсылка.ДополнительныеРеквизитыИСведения") Тогда
		ВладелецЗначений = Элемент.ТекущаяСтрока;
		ЗаполнитьЗначенияДополнительныхРеквизитов(ВладелецЗначений);
	Иначе
		ЗначенияДополнительныхРеквизитов.ПолучитьЭлементы().Очистить();
	КонецЕсли;
КонецПроцедуры
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокДополнительныхРеквизитов

&НаКлиенте
Процедура СписокДополнительныхРеквизитовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
Процедура КомандаДалее(Команда)
	...
	Если ИндексСтраницы = 0
		И ТипЗнч(Элементы.СписокДополнительныхРеквизитов.ТекущаяСтрока) <> Тип("ПланВидовХарактеристикСсылка.ДополнительныеРеквизитыИСведения") Тогда
		ТекстПредупреждения = НСтр("ru = 'Выберите элемент, а не группу!
			|Для раскрытия групп используйте ""Ctrl"" и стрелки вверх/вниз или клавиши ""+"" и ""-"" на дополнительной клавиатуре.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСтраницуВыбора()
	...
	Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			СписокДополнительныхРеквизитов, "ЭтоДополнительноеСведение", (ЭтоДополнительноеСведение = 1), , , Истина);
	КонецЕсли;
	
	Если Не ИспользоватьОбщиеДополнительныеРеквизитыИСведения Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			СписокДополнительныхРеквизитов, "НаборСвойств", , ВидСравненияКомпоновкиДанных.Заполнено, , Истина);
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокДополнительныхРеквизитов, "ВладелецДополнительныхЗначений", ,
		ВидСравненияКомпоновкиДанных.НеЗаполнено, , Истина);
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		СписокДополнительныхРеквизитов,
		"ПредставлениеГруппировкиОбщихСвойств",
		НСтр("ru = 'Общие (для нескольких наборов)'"),
		Истина);
	
	// Группировка свойств по наборам.
	ГруппировкаДанных = СписокДополнительныхРеквизитов.КомпоновщикНастроек.Настройки.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		...
		Элементы.СписокДополнительныхРеквизитов.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Иначе
			...
			ВыбранныйЭлемент = Элементы.СписокДополнительныхРеквизитов.ТекущиеДанные;
			Если ВыбранныйЭлемент = Неопределено Тогда
				ВладелецДополнительныхЗначений = ПереданныеПараметрыФормы.ВладелецДополнительныхЗначений;
			Иначе
				ВладелецДополнительныхЗначений = Элементы.СписокДополнительныхРеквизитов.ТекущиеДанные.Ссылка;
			КонецЕсли;
		КонецЕсли;
		Элементы.КомандаДалее.Заголовок = НСтр("ru = 'Далее >'");
...
Процедура ПриСменеСтраницы(Направление, ОсновнаяСтраница, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.ВыборДействия Тогда
		Если Направление = "Вперед" Тогда
			ВыбранныйЭлемент = Элементы.СписокДополнительныхРеквизитов.ТекущиеДанные;
			ПереданныеПараметрыФормы.ВладелецДополнительныхЗначений = ВыбранныйЭлемент.Ссылка;
			ЗаполнитьСписокДействийПриДобавленииРеквизита();
		КонецЕсли;
