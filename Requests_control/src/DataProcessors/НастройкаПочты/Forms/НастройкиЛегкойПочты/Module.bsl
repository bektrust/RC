...
Процедура ОбновитьТаблицуПрофилейДляОтправки()
	...
	Для каждого ХранилищеСтрока Из ПрофилиДляОтправкиХранилище.ВыгрузитьЗначения() Цикл
		...
		Иначе
			...
			ДобавлятьПрофиль = Истина;
			Если ТипЗнч(ХранилищеСтрока.Наименование) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") Тогда
				ДобавлятьПрофиль = Не ХранилищеСтрока.Наименование.ПометкаУдаления;
			КонецЕсли;	
			
			Если ДобавлятьПрофиль Тогда
				ПрофилиСтрока = ПрофилиДляОтправки.Добавить();
...
Процедура ДобавитьПрофильSMTPPOP3(Команда = Неопределено)
	...
	ОткрытьФорму("Справочник.УчетныеЗаписиЭлектроннойПочты.Форма.ФормаЭлемента",, УникальныйИдентификатор,,,,
	...
	Для каждого Элемент Из ПрофилиДляОтправкиХранилище Цикл
		Если Элемент.Представление = ПрофильСтр Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В списке профилей для отправки уже присутствует профиль %1'"),
				ПрофильСтр);
